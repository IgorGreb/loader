...
## Підготовка до CodeCanyon

1. Переконайтесь, що виконані тести:
   ```bash
   flutter analyze
   flutter test
   ```
2. Заповніть зміни у `CHANGELOG.md` та додайте потрібні скріншоти/відео (папку `documentation/` залишайте всередині архіву).
3. Запустіть скрипт упаковки:
   ```bash
   ./tool/package_release.sh
   ```
   Він виконає `flutter clean`, видалить тимчасові каталоги та збере архів `aurora-loader.zip` на рівень вище.
4. Завантажте zip на CodeCanyon як основний файл і додайте прев’ю (590×300) та додаткові скріншоти ≥900×600.

Детальний гайд для покупців та рев’юверів знаходиться в `documentation/index.md`.
